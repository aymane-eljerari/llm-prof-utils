...
for quant in "${quants[@]}"; do
  for split_mode in "${split_modes[@]}"; do
    # Run the llama-batched-bench command and capture the output in 'llama.log'
    OUTPUT=$(llama.cpp/llama-batched-bench -m llama.cpp/models/llama3/Meta-Llama-3-8B-Instruct-$quant.gguf -b 2048 -ub 512 -npp 64,128,256,512 -ntg 64,128 -npl 8,16,32,64 -ngl 50 -pps -sm $split_mode 2>&1)

    # Pass the content of 'llama.log' to the Python script
    python3 generate_tables.py "$quant" "$split_mode" "${OUTPUT}"
  done
done
